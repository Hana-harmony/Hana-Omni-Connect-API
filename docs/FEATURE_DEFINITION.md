...
| 영속 뉴스 이벤트 저장소 | `Hana-OmniLens-API` | canonical URL, content hash, AI duplicate key, cluster key, source published time으로 진짜 신규 이벤트를 판정하고 DB에 저장 | Planned |
...
| 뉴스 REST 목록·상세 | `Hana-OmniLens-API` | 협력사가 종목 상세 news 탭에 사용할 market-wide 뉴스·공시 목록과 전문 상세를 조회 | Planned |
| 뉴스 WebSocket 이벤트 | `Hana-OmniLens-API` | 새 이벤트 처리 완료 시 협력사/종목 topic으로 저장된 이벤트 ID와 요약 payload를 송신 | Planned |
...
| Hana alert WebSocket 송신 | Done for v1, market-wide full-content event store/REST/WebSocket v2 Planned |
